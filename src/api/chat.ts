...
const QWEN3_API_KEY = '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';
...
IMPORTANT: You must ONLY respond to questions and requests that are directly related to your specific role and purpose as defined above. If a user asks about topics outside your domain or tries to get you to perform tasks unrelated to your role, politely decline and redirect them back to your intended purpose. Do not provide information or assistance on unrelated topics.`;
    ...
    if (!response.ok) {
      console.warn(`API error: ${response.status} ${response.statusText}, falling back to mock response`);
...
function isMessageRelevantToRole(message: string, systemPrompt: string): boolean {
  const lowerMessage = message.toLowerCase();
  const lowerPrompt = systemPrompt.toLowerCase();
  
  // Define role-specific keywords
  const roleKeywords: { [key: string]: string[] } = {
    'customer support': ['issue', 'problem', 'help', 'support', 'bug', 'error', 'complaint', 'refund', 'billing', 'account', 'service'],
    'portfolio': ['work', 'project', 'experience', 'skill', 'portfolio', 'resume', 'background', 'education', 'career'],
    'faq': ['question', 'how', 'what', 'when', 'where', 'why', 'faq', 'frequently'],
    'feedback': ['feedback', 'review', 'opinion', 'suggestion', 'rating', 'comment']
  };
  ...
  return relevantKeywords.some(keyword => lowerMessage.includes(keyword)) || 
         lowerMessage.includes('hello') || lowerMessage.includes('hi') || lowerMessage.includes('help');
}
...
function getMockResponse(request: ChatRequest): ChatResponse {
  ...
  if (request.system_prompt.toLowerCase().includes('customer support')) {
    response = "Thank you for contacting our support team. I'm here to help resolve your issue quickly and efficiently.";
  } else if (request.system_prompt.toLowerCase().includes('portfolio')) {
    response = "Welcome to my portfolio! I'm excited to share my work and experience with you.";
  } else if (request.system_prompt.toLowerCase().includes('faq')) {
