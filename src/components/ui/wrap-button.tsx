...
const WrapButton: React.FC<WrapButtonProps> = ({
  ...
  const Inner = (
    <div
      className={cn(
        "group cursor-pointer border border-border bg-background gap-2 h-[50px] flex items-center p-[8px] rounded-full",
        className
      )}
    >
      {/* Inner colored capsule */}
      <div className="border border-border bg-primary h-[30px] rounded-full flex items-center justify-center text-primary-foreground px-2">
        {/* Optional spinning globe for non-link version */}
        {!href && <Globe className="mx-1 animate-spin" />}
